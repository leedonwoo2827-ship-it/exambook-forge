...
0. **백업(기존 03 이동).** `03/`에 기존 `summary_*`·`assets/`가 있으면 통째로
   `03/_backup_<YYYYMMDD-HHMM>/`로 **이동**한 뒤 새로 생성한다(덮어쓰기 아님, 이력 보존).
   예: `mkdir 03/_backup_20260727-1130 && move 03/summary_* 03/assets 03/_backup_20260727-1130/`.
   `_backup_*` 폴더 자체는 다시 백업하지 않는다.
1. **수집(전체 병합).** `01/`의 모든 기출 회차 + `02/`의 모든 자사 회차 해설을 과목별로 모아
   tags/개념으로 군집화한다(특정 회차만이 아니라 **전 회차 종합**).
